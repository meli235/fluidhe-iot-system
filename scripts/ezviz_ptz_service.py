...
def do_login(account="anugrahtriplecycle@gmail.com", password="Eva1Yosep2", mfa_code=None):
    try:
...
def do_move(direction, duration=0.6):
    ...
    if not client:
        ...
        client = EzvizClient(account="anugrahtriplecycle@gmail.com", password="Eva1Yosep2", url=REGION_URL)
        try:
            res = client.login()
            with open(SESSION_FILE, 'w') as f:
                json.dump(res, f)
    ...
    try:
        ...
        try:
            c_retry = EzvizClient(account="anugrahtriplecycle@gmail.com", password="Eva1Yosep2", url=REGION_URL)
...
if __name__ == "__main__":
    ...
    if cmd == "login":
        ...
        pwd = sys.argv[3] if len(sys.argv) > 3 else "Eva1Yosep2"
