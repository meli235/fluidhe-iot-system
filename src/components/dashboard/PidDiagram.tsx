...
export const PidDiagram: React.FC<PidDiagramProps> = ({
  ...
  return (
    <div className="bg-slate-50/80 rounded-2xl border border-slate-200/90 p-4 space-y-4 relative overflow-hidden shadow-sm">
      {/* Subheader Badge */}
      <div className="flex flex-wrap justify-between items-center gap-2 pb-2 border-b border-slate-200/70">
        <span className="px-2.5 py-1 rounded-xl text-xs font-extrabold uppercase shadow-xs bg-sky-50 text-sky-700 border border-sky-200">
          Mode Aktif: {diagramMode} {titleExtra}
        </span>
      </div>
        ...
        <svg viewBox="0 0 940 460" className="w-full h-auto min-w-[760px] font-sans">
          ...
          {isCounter ? (
            <>
              ...
              <text x="690" y="415" textAnchor="middle" className="text-[10px] font-extrabold fill-cyan-700">Air Dingin (Cold Water In)</text>
            ...
            <>
              ...
              <text x="450" y="415" textAnchor="middle" className="text-[10px] font-extrabold fill-cyan-700">Air Dingin (Cold Water In)</text>
      ...
      <div className="p-3.5 bg-white rounded-xl border border-slate-200/90 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs shadow-sm">
        <div>
          <span className="text-[10px] font-semibold text-slate-500 block">Penurunan Hot Fluid (ΔT_hot):</span>
          <strong className="text-amber-700 font-extrabold text-sm">{(latestData.ti1 - latestData.ti2).toFixed(1)} °C</strong>
        </div>
        <div>
          <span className="text-[10px] font-semibold text-slate-500 block">Kenaikan Cold Fluid (ΔT_cold):</span>
          <strong className="text-sky-700 font-extrabold text-sm">{(latestData.ti4 - latestData.ti3).toFixed(1)} °C</strong>
        </div>
        <div>
          <span className="text-[10px] font-semibold text-slate-500 block">Estimasi LMTD ({diagramMode}):</span>
          <strong className="text-slate-800 font-extrabold text-sm">{lmtdVal.toFixed(2)} °C</strong>
        </div>
        <div>
          <span className="text-[10px] font-semibold text-slate-500 block">Status Katup Solenoid:</span>
          <span className="inline-block font-bold text-[11px] px-2 py-0.5 rounded bg-slate-100 text-slate-700 border border-slate-200">
